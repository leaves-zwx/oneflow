...
Maybe<void> CheckpointingPass::Apply(const OpGraph& op_graph, JobBuilder* job_builder) const {
  ...
  for (auto& subgraph : checkpointing_subgraphs) {
    ...
    // if
    // (!first_bw_consumer->parallel_desc().EqualsIgnoringHierarchy(end_op_node->parallel_desc())) {
    //   std::string lbn = "";
    //   LogicalBlobId lbi;
    ...
    //   auto id_op = user_op::UserOpConfWrapperBuilder(kCheckpointingIdentityOpName +
    //   NewUniqueId())
    //                    .Op("identity")
    //                    .Input("in", lbn)
    //                    .Output("out")
    ...
    //   id_op.op_conf())); end_op_name = id_op.op_name();
    // }
