...
DEFINE_ENV_BOOL(DISABLE_ACTIVATION_CHECKPOINTING_CTRL_EDGE, false);
...
Maybe<void> CheckpointingPass::Apply(const OpGraph& op_graph, JobBuilder* job_builder) const {
  ...
  for (auto& subgraph : checkpointing_subgraphs) {
    ...
    if (!EnvBool<DISABLE_ACTIVATION_CHECKPOINTING_CTRL_EDGE>()) {
      ...
      if (!first_bw_consumer->parallel_desc().EqualsIgnoringHierarchy(
              end_op_node->parallel_desc())) {
        std::string lbn = "";
        LogicalBlobId lbi;
        ...
        for (const std::string& ibn : end_op_edge->lbi2ibns().at(lbi)) {
          std::string old_lbn = ReplaceInputLbnInOpCustomizedConf(
              &(total_bw_consumers_op_name2conf.at(first_bw_consumer->op().op_name())), ibn,
              id_out);
          CHECK_EQ(old_lbn, lbn);
        }

        JUST(job_builder->AddOp(first_bw_consumer->parallel_desc().parallel_conf(),
                                id_op.op_conf()));
