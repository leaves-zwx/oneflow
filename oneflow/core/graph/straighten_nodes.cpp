...
void StraightenNodesALAP(TaskGraph* task_graph, std::vector<TaskNode*>* ordered_task_nodes) {
  // 1. Find all sink nodes
  std::queue<const TaskNode*> queued_nodes;
  task_graph->ForEachNode([&](TaskNode* node) -> void {
    if (node->out_edges().empty()) { queued_nodes.push(node); }
  });

  // 2. Decide all nodes's depth
  int depth_count = 0;
  HashMap<const TaskNode*, int> node2depth;
  // nullptr indicate separator, nodes following it belong to the next depth
  queued_nodes.push(nullptr);
  while (!queued_nodes.empty()) {
    const TaskNode* cur_node = queued_nodes.front();
    queued_nodes.pop();
    if (cur_node == nullptr) {
    ...
    if (depth_updated) {
      cur_node->ForEachNodeOnInEdge([&](TaskNode* parent) -> void { queued_nodes.push(parent); });
    }
  }

  // 3. Traverse by depth
  int max_depth = 0;
  HashMap<int, std::vector<TaskNode*>> depth2nodes;
  // 3.1 collect depth nodes
  task_graph->TopoForEachNode([&](TaskNode* node) -> void {
  ...
  for (int d = max_depth; d >= 0; --d) {
    ...
    for (TaskNode* node : depth_nodes) { ordered_task_nodes->push_back(node); }
  }
}
